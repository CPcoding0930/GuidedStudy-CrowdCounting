...
class SpatialTransformer_2DTo2D_real(Layer):
    ...
    def compute_output_shape(self, input_shape):
        ...
        return (int(input_shape[0] / patch_num),
                ...
                int(output_size[0]),
                int(output_size[1]),
                int(input_shape[-1]))  # no add the depth too.
    ...
    def proj_2DTo2D(self, view, inputs):
        ...
        grid_rangeX = (grid_rangeX * 4 - bbox[0]) * resolution_scaler
        grid_rangeX = grid_rangeX + 1
        grid_rangeX = np.expand_dims(grid_rangeX, 1)

        grid_rangeY = (grid_rangeY * 4 - bbox[1]) * resolution_scaler
        ...
        if view == 1:
            view = 'view1'
            view_gp_mask = np.load('/Users/pcp25/Downloads/CityStreet_dataset/ROI_maps/ROIs/ground_plane/view1_GP_mask.npz')
        if view == 2:
            view = 'view2'
            view_gp_mask = np.load('/Users/pcp25/Downloads/CityStreet_dataset/ROI_maps/ROIs/ground_plane/view2_GP_mask.npz')
        if view == 3:
            view = 'view3'
            view_gp_mask = np.load('/Users/pcp25/Downloads/CityStreet_dataset/ROI_maps/ROIs/ground_plane/view3_GP_mask.npz')
    ...
    def proj_splat(self, inputs):
        with tf.variable_scope('ProjSplat'):
            ...
            with tf.name_scope('BilinearInterp'):
                ...
                self.Ibilin = Ibilin  # no mask needed.
